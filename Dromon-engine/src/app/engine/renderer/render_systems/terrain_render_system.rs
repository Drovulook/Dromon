...
use crate::app::engine::{
    renderer::{descriptors::DescriptorHandler, terrain::TerrainVertex},
...
impl TerrainRenderSystem {
    ...
    pub(crate) fn record_shadow_terrain(
        ...
        unsafe {
            ...
            // set 0 : UBO (light_view_proj). Aucun draw tant que le terrain n'a pas
            // de géométrie — la pipeline est en place, prête à projeter des ombres.
            self.context.device.cmd_bind_descriptor_sets(
                command_buffer,
                vk::PipelineBindPoint::GRAPHICS,
                self.pipeline_layout,
